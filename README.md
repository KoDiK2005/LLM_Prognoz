...
- `POST /api/v1/forecasts` — ставит прогноз по `dataset_id` на
  `horizon` точек вперёд в очередь (см. «Очередь задач» ниже), сразу
  возвращает `202` со статусом `pending`; результат — через `GET` по
  тому же `run_id`.
- Модель — Holt-Winters (statsmodels), а не Prophet: даёт тренд +
  ...
  на два полных цикла.
- Auth обязателен для всех эндпоинтов кроме `/auth/*` и `/health` — см.
  раздел «Auth» ниже.
...
- 23 теста: регистрация/логин/`/me`/дубликат email, rate-limit на login
  (10 попыток → `429`), загрузка датасета (валидный/невалидный CSV, мало
  точек, превышен лимит размера, изоляция по org), создание/чтение
  прогона (cross-org 404, история по датасету), генерация инсайтов
  (mock-провайдер → `completed`, недоступная Ollama → `failed`,
  недостроенный прогон → 400), недоступность очереди → `503` и прогон
  помечается `failed` вместо вечного `pending`, health-check, S3-backend
  для датасетов (через `moto`, без реальных AWS-кредов).
- Redis для rate-limit использует отдельный логический индекс БД (`/15`,
  `TEST_REDIS_URL`), не пересекающийся с тем, что использует Arq —
  автоочистка перед каждым тестом (`FLUSHDB`) не трогает дев-очередь.
...
- `POST /auth/login` и `POST /auth/register` ограничены 10
  запросами/минуту на IP (`app/services/rate_limit.py`, fixed-window
  счётчик в Redis) — раньше пароль можно было перебирать без всякого
  ограничения. Превышение лимита → `429`.

## Хранилище датасетов

`app/services/storage.py` — выбираемый backend, переключается через
`STORAGE_BACKEND` (по умолчанию `local`), вызовы `save()`/`read()` на
стороне API/воркера не меняются независимо от backend'а:

- `local` — диск под `./storage` (как было с итерации 3). Просто для
  разработки, но датасет не переживёт пересборку volume.
- `s3` — настоящий S3 или любой S3-совместимый эндпоинт
  (`S3_ENDPOINT_URL`, например self-hosted MinIO — бесплатная
  альтернатива без облака) через `boto3`. AWS-креды читаются из
  стандартных `AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`, не из
  настроек приложения. И сохранение, и чтение идут через
  `asyncio.to_thread`, чтобы синхронный сетевой I/O `boto3` не блокировал
  event loop.
- Тесты на `s3`-backend — через `moto` (мок S3 в памяти, без реальных
  AWS-кредов и расходов): юнит-тест на raw save/read, и полный
  HTTP-флоу (загрузка датасета → прогон → `completed`) с
  `storage_backend="s3"`.

## Статус

Итерации 10–12: продакшенизация. Backend — pytest (23 теста, реальная
Postgres), обработка отказа очереди задач, настраиваемый CORS, лимит на
размер загрузки, health-check с проверкой БД, rate-limit на auth,
выбираемый storage backend (local/S3, протестирован через moto).
Frontend — Vitest-тесты на `apiFetch`, авто-логаут на истёкший токен,
видимые ошибки загрузки. CI на GitHub Actions (backend tests + миграции
+ frontend tsc/eslint/vitest/build) гоняется на каждый push/PR в
`master`; Dependabot держит зависимости свежими без шума от мажорных
бампов. Прошла внутренний security-review без находок. Self-hosted
Ollama остаётся основным бесплатным LLM-провайдером для разработки.
Дальше — на выбор: реальные платные ключи (когда понадобится), или
дальнейший UI-полиш.
