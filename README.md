...
- 17 тестов: регистрация/логин/`/me`/дубликат email, загрузка датасета
  (валидный/невалидный CSV, мало точек, изоляция по org), создание/чтение
  ...
  помечается `failed` вместо вечного `pending`.
...
Итерация 10: тесты backend (pytest, 17 тестов, реальная Postgres) +
обработка отказа очереди задач и настраиваемый CORS. Self-hosted Ollama
остаётся основным бесплатным LLM-провайдером для разработки. Дальше —
на выбор: реальные платные ключи (когда понадобится), CI (GitHub
Actions, прогон тестов на пуш), или дальнейший UI-полиш.
