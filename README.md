...
- **LLM отвечает за интерпретацию.** Объяснение тренда, ответы на вопросы
  пользователя о прогнозе (данные ряда — прямо в контексте промпта, а не
  через tool-use/function calling: у трёх разных провайдеров — три
  разных несовместимых API для вызова функций, а ряд небольшой и
  целиком помещается в промпт, усложнять не было смысла), сравнение
  "точек зрения" разных провайдеров на один и тот же прогноз.
...
- `POST /api/v1/forecasts/{run_id}/ask` — свободный вопрос про прогноз
  (`{"question": "...", "provider": "openai"}`), синхронный ответ (без
  очереди — пользователь явно ждёт ответ, как в чате). История, точки
  прогноза и метаданные модели подставляются в промпт целиком
  (до 60 последних точек истории), без персистентности — каждый вопрос
  независим, истории диалога нет. Сбой провайдера возвращает текст
  ошибки как ответ (`200`), не `5xx` — тот же паттерн, что у `/insights`.
...
Итерации 10–13: продакшенизация + UI-полиш + закрытие архитектурного
долга. Backend — pytest (30 тестов, реальная Postgres), обработка
отказа очереди задач, настраиваемый CORS, лимит на размер загрузки,
health-check с проверкой БД, rate-limit на auth, выбираемый storage
backend (local/S3, протестирован через moto), удаление датасетов/
прогонов с каскадом на уровне БД, эндпоинт свободных вопросов про
прогноз (`/forecasts/{id}/ask`) — закрывает обещание из архитектурного
раздела этого README, которое не было реализовано с итерации 1.
Frontend — Vitest-тесты на `apiFetch`, авто-логаут на истёкший токен,
видимые ошибки загрузки, оси и тултип на графике прогноза, превью CSV
с авто-определением колонок перед загрузкой, UI для удаления и для
вопросов про прогноз (inline-подтверждение вместо `window.confirm()` —
он блокирует рендерер и реально вешал страницу при тестировании). CI
на GitHub Actions (backend tests + миграции + frontend
...
на выбор: реальные платные ключи (когда понадобится), или multi-user
org-инвайты/refresh-токены (требуют продуктового решения, не чистый
technical debt fix).
