...
- `/dashboard` — единая страница: загрузка CSV (`DatasetUpload`), выбор
  ...
  — лёгкий SVG без сторонних chart-библиотек), генерация LLM-инсайтов с
  выбором провайдеров и поллингом (`InsightsPanel`, раз в 2с, пока есть
  `pending`).
...
- Проверено вручную в браузере (Chrome MCP): регистрация → загрузка
  датасета → прогноз через очередь → график → генерация инсайтов →
  logout → редирект неавторизованного пользователя на `/login`. Сама
  загрузка файла через `<input type="file">` не была прогнана через
  браузерный клик (sandbox не даёт грузить произвольные локальные файлы
  в headless-сессию) — датасет для теста загружен через API тем же
  пользователем, форма проверена статически (`tsc`, `eslint`, `next build`).

## Статус

Итерация 8: фронтенд UI (Next.js) — auth, загрузка датасетов, прогноз
с поллингом, график, LLM-инсайты. Дальше — на выбор: реальные API-ключи
LLM-провайдеров, список прошлых прогонов, или сравнение интерпретаций
нескольких LLM в одном UI.
